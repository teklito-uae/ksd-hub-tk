                ...
                <form onSubmit={handleRegister} className="space-y-6">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2 md:col-span-2">
                        ...
                        <Input required value={name} onChange={(e) => setName(e.target.value)} placeholder={role === 'pro' ? "Your name" : "Official business name"} className="h-14 rounded-2xl border-gray-100 bg-gray-50 focus-visible:bg-white focus-visible:ring-primary/20 transition-all font-medium" />
                      </div>

                      <div className="space-y-2">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-secondary ml-1">WhatsApp Number</Label>
                        <Input required type="tel" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder="+91 00000 00000" className="h-14 rounded-2xl border-gray-100 bg-gray-50 focus-visible:bg-white focus-visible:ring-primary/20 transition-all font-medium" />
                      </div>
                   ...
                   <Button 
                    type="submit" 
                    disabled={isLoading}
                    className="w-full h-14 rounded-2xl bg-secondary hover:bg-black font-black uppercase tracking-widest shadow-xl shadow-black/5 active:scale-[0.98] transition-all"
                   >
                      {isLoading ? 'Creating Account...' : 'Create My Account'} <CheckCircle2 className="ml-2 size-4" />
                   </Button>
