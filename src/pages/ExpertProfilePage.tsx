...
import { dummyPros, dummyBusinesses } from '@/lib/dummy-data';
...
export function ExpertProfilePage() {
  ...
  const pro = dummyPros.find((p) => p.slug === slug);
  ...
  const linkedBusinesses = (pro.business_ids || []).map(bid => dummyBusinesses.find(b => b.id === bid)).filter(Boolean);
              ...
              <div className="mt-4 md:mt-8">
                 ...
                 <div className="flex flex-wrap items-center gap-x-5 gap-y-2.5 mt-4 md:mt-5">
                    ...
                    <div className="flex items-center gap-2 text-sm md:text-base text-secondary font-black">
                       <Star className="size-4 fill-primary text-primary" /> {pro.rating} <span className="text-muted-foreground font-medium underline decoration-primary/20 decoration-2 underline-offset-4">{pro.experience} Experience</span>
                    </div>
                 </div>
              </div>
          ...
          <div className="lg:col-span-8 space-y-4 md:space-y-6">
            ...
            <Card className="p-6 md:p-10 rounded-2xl md:rounded-3xl border-gray-100 bg-white shadow-sm ring-1 ring-black/[0.01]">
               ...
               <div className="prose prose-sm md:prose-base max-w-none text-secondary/80 leading-[1.7] space-y-4 font-medium">
                  <p>{pro.bio}</p>
                  <p>
                    Dedicated to providing excellence in {pro.profession} across {pro.location} and surrounding areas. 
            ...
            <Card className="p-6 md:p-10 rounded-2xl md:rounded-3xl border-gray-100 bg-white shadow-sm ring-1 ring-black/[0.01]">
               ...
               <div className="space-y-10 md:space-y-12">
                  {pro.portfolio.map((_, i) => (
                    <div key={i} className="flex gap-4 md:gap-6 group relative">
                       {/* Timeline Decor */}
                       {i < pro.portfolio.length - 1 && <div className="absolute left-[23px] top-12 bottom-[-48px] w-0.5 bg-gray-50" />}
                       ...
                       <div className="space-y-2 md:space-y-3 flex-1 pb-2">
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-1">
                             ...
                             <span className="text-[10px] font-black text-muted-foreground uppercase tracking-wider">{pro.location} • 2024</span>
                          </div>
                ...
                <Card className="p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] border-gray-100 bg-white shadow-sm">
                   ...
                   <div className="flex flex-wrap gap-2">
                      {pro.skills.map(skill => (
                         <Badge key={skill} variant="secondary" className="rounded-xl px-4 py-2 border-none bg-gray-50 text-secondary text-[11px] font-black hover:bg-primary/5 hover:text-primary transition-colors cursor-default">
                           {skill}
                         </Badge>
                      ))}
                   </div>
